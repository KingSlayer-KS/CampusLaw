...
import {
  Send, Scale, AlertTriangle, ExternalLink, MessageSquare, Loader2, Sun, Moon, Plus,
} from "lucide-react";
...
import { useChatHistory, type Confidence, type LegalResponse } from "@/hooks/useChatHistory";
...
async function apiFetch(path: string, init: RequestInit = {}) {
  ...
  console.log("[FE apiFetch] →", {
    ...
    body: (() => {
      try { return init.body ? JSON.parse(String(init.body)) : undefined; } catch { return init.body; }
    })(),
  });
...
/* ────────────────────────────────────────────────────────────────────────────
   ...
   ──────────────────────────────────────────────────────────────────────────── */
type FeedbackReason =
  | "notAccurate" | "missingCitation" | "unclear" | "wrongJurisdiction" | "offTopic" | "other";
...
function FeedbackBar({
  traceId, topic, question, summary,
}: { traceId: string; topic: "tenancy" | "traffic"; question: string; summary?: string }) {
  ...
  const toggle = (k: FeedbackReason) =>
    setReasons((xs) => (xs.includes(k) ? xs.filter((x) => x !== k) : [...xs, k]));

  const submit = async (helpful: boolean) => {
    setSending(true);
    try {
      await sendFeedback({
        traceId, topic, question, helpful,
  ...
  if (state === "sent") return <p className="text-xs text-muted-foreground">Thanks for the feedback!</p>;

  return (
    <div className="mt-3 rounded-lg border border-border p-3">
      <div className="flex items-center gap-2">
        <span className="text-sm">Was this helpful?</span>
        <button className="text-xs rounded-full px-2 py-1 bg-primary text-primary-foreground" disabled={sending} onClick={() => submit(true)}>👍 Yes</button>
        <button className="text-xs rounded-full px-2 py-1 bg-muted text-foreground" disabled={sending} onClick={() => setState("down")}>👎 No</button>
      </div>
      {state === "down" && (
        <div className="mt-3 space-y-3">
          ...
          <div className="flex justify-end">
            <button className="text-xs rounded-md px-3 py-2 bg-primary text-primary-foreground" disabled={sending} onClick={() => submit(false)}>
...
export function ChatInterface() {
  ...
  type Session = { id: string; title: string; backendId?: string; messages: any[] };

function ensureLocalSession(localId: string, setSessions: React.Dispatch<React.SetStateAction<Session[]>>) {
  let created = false;
  setSessions(prev => {
    const found = prev.find(s => s.id === localId);
    if (found) return prev;
    created = true;
    return [...prev, { id: localId, title: "New chat", messages: [] }];
  });
  if (created) console.log("[ensureLocalSession] created local session", { localId });
}
  ...
  function toLegalResponse(question: string, raw: any): LegalResponse {
    if (Array.isArray(raw.short_answer) || Array.isArray(raw.what_the_law_says)) {
      return {
        ...
        caveats: raw.caveats ?? [],
        sources: (raw.sources ?? []).map((s: any) => (typeof s === "string" ? s : s?.url)).filter(Boolean),
        followups: raw.followups ?? [],
        confidence: (["high", "medium", "low"] as Confidence[]).includes(raw.confidence)
          ? raw.confidence
          : "low",
      };
    }
    ...
    return {
      ...
      caveats: ans?.caveats ?? [],
      sources: (ans?.sources ?? []).map((s: any) => (typeof s === "string" ? s : s?.url)).filter(Boolean),
      followups: ans?.followups ?? [],
      confidence: (["high", "medium", "low"] as Confidence[]).includes(ans?.confidence)
        ? ans.confidence
        : "low",
    };
  }
...
async function ensureBackendSession(
  ...
  if (backendCreateLocks.has(localId)) {
    console.log("[ensureBackendSession] lock hit → returning existing promise", { localId });
    return backendCreateLocks.get(localId)!;
  }

  const p = (async () => {
    // A) make sure a local session exists
    console.log("[ensureBackendSession] step A: ensureLocalSession");
    ensureLocalSession(localId, setSessions);

    // B) if we already have a backendId in state, use it
    const inState = sessions.find(s => s.id === localId)?.backendId;
    if (inState) {
      console.log("[ensureBackendSession] step B: found in state", { localId, backendId: inState });
      return inState;
    }
    ...
    if (cached) {
      console.log("[ensureBackendSession] step C: found cached backendId", { localId, backendId: cached });
      setSessions(prev =>
        prev.map(s => s.id === localId ? { ...s, backendId: cached } : s)
      );
      return cached;
    }

    // D) create on the server
    console.log("[ensureBackendSession] step D: creating backend session", { localId });
    ...
    const backendId = created.backendId ?? created.id;
    if (!backendId) {
      console.error("[ensureBackendSession] ERROR: backend did not return id", { localId, created });
    ...
    console.log("[ensureBackendSession] step E: persisted + updated state", { localId, backendId });
    return backendId;
  })();
  ...
  try {
    ...
    console.log("[ensureBackendSession] success", { localId, backendId: result });
    return result;
  } catch (e) {
    console.error("[ensureBackendSession] failed", { localId, error: (e as Error).message });
    throw e;
  } finally {
  ...
  /* ──────────────────────────────────────────────────────────────────────────
     ...
     const callLegalAPI = async (question: string, serverSessionId: string) => {
      ...
      console.log("[FE callLegalAPI] sending", { question, topic, serverSessionId, hasToken: !!token });
      ...
      const raw: any = (() => { try { return JSON.parse(txt); } catch { return {}; } })();
  ...
  /* ──────────────────────────────────────────────────────────────────────────
     ...
     const handleSendMessage = async () => {
      ...
      // Title candidate (don't read `active` immediately after setActiveId)
      const titleCandidate = (sessions.find((s: any) => s.id === sid)?.title && sessions.find((s: any) => s.id === sid)?.title !== "New chat")
        ? sessions.find((s: any) => s.id === sid)!.title
        : q.slice(0, 40);
    
      // 2) Ensure a backend session id
      let serverSessionId = sessions.find((s: any) => s.id === sid)?.backendId as string | undefined;
    
      if (!serverSessionId) {
        try {
          // NOTE: use the signature that matches your helper
          serverSessionId = await ensureBackendSession(sid, sessions, setSessions);
          console.log("[FE send] backend session ready", { sid, serverSessionId });
        } catch (e) {
      ...
      try {
        ...
        }).catch(err => console.warn("[FE send] save assistant message failed", err));
    
        // 6) Auto-title once (use sid, not active.id)
        const nowSession = sessions.find((s: any) => s.id === sid);
        if (nowSession && (!nowSession.title || nowSession.title === "New chat")) {
          const newTitle = titleCandidate;
          renameSession(sid, newTitle);
          apiFetch(`/history/${serverSessionId}`, {
            method: "PATCH",
            body: JSON.stringify({ title: newTitle }),
          }).catch(err => console.warn("[FE send] rename backend session failed", err));
        }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Badge className={getConfidenceColor(response.confidence)}>
          {response.confidence[0].toUpperCase() + response.confidence.slice(1)} Confidence
        </Badge>
      </div>
        ...
        <Alert className="border-amber-200 bg-amber-50 dark:border-amber-900/40 dark:bg-amber-900/20">
          ...
          <AlertDescription>
            <div className="space-y-2">
              <div className="font-semibold text-amber-800 dark:text-amber-200">Important Limitations</div>
              <ul className="space-y-1">
                {response.caveats.map((caveat, idx) => (
                  <li key={idx} className="flex items-start space-x-2 text-sm text-amber-800 dark:text-amber-100">
            ...
            {response.what_the_law_says.map((law, idx) => (
              <div key={idx} className="border-l-2 border-border pl-4">
                <div className="flex items-center space-x-2 mb-2">
                  ...
                  <Badge variant="outline" className="text-xs">s.{law.section}</Badge>
                </div>
                <blockquote className="text-sm text-muted-foreground italic mb-2">“{law.quote}”</blockquote>
                <a href={law.url} target="_blank" rel="noopener noreferrer" className="text-xs text-primary hover:underline flex items-center space-x-1">
          ...
          <div className="flex flex-wrap gap-2">
            {response.sources.map((url, idx) => (
              ...
              >
                ...
                <span className="truncate max-w-[220px] sm:max-w-[300px]">{shortLabel(url)}</span>
              </a>
            ))}
          </div>
        </Card>
  ...
  return (
    <div className="flex min-h-dvh bg-background text-foreground">
      ...
      <HistorySidebar
        ...
        onNew={() => { const id = createSession(); setActiveId(id); setSidebarOpen(false); }}
            ...
            <div className="w-full max-w-3xl text-center space-y-6">
              ...
              <form onSubmit={(e) => { e.preventDefault(); handleSendMessage(); }} className="w-full">
                <div className="relative">
                  <button type="button" aria-label="Add" className="absolute left-2 top-1/2 -translate-y-1/2 rounded-full p-2 text-muted-foreground hover:text-foreground">
                  ...
                  <Input
                    ...
                    disabled={isLoading}
                  />
                  <Button type="submit" size="icon" disabled={!inputValue.trim() || isLoading} className="absolute right-2 top-1/2 -translate-y-1/2 h-10 w-10 rounded-full" aria-label="Send">
                  ...
                  <div key={m.id} className={`flex ${m.type === "user" ? "justify-end" : "justify-start"}`}>
                    ...
                    >
                      {m.type === "user" ? (
                        ...
                        <div className="space-y-4">
                          {m.content?.trim() && <p className="text-sm text-muted-foreground">{m.content}</p>}
                          {m.legalResponse && (
                            <>
                              <LegalResponseDisplay response={m.legalResponse} />
                              <FeedbackBar
                                traceId={m.legalResponse.traceId}
                                topic={topic}
                                question={m.legalResponse.question}
                                summary={m.legalResponse.short_answer?.slice(0, 2).join(" • ").slice(0, 200)}
                              />
                            </>
                          )}
                        </div>
                  ...
                  <div className="flex justify-start">
                    <div className="bg-card text-card-foreground border border-border rounded-lg p-4 shadow-sm">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-sm text-muted-foreground">Researching Ontario law...</span>
                      </div>
                    </div>
                  </div>
                )}
                ...
                <form onSubmit={onSubmit}>
                  <div className="relative">
                    ...
                    <Input
                      ...
                      disabled={isLoading}
                    />
                    <Button type="submit" size="icon" disabled={!inputValue.trim() || isLoading} className="absolute right-2 top-1/2 -translate-y-1/2 h-9 w-9 rounded-full" aria-label="Send">
                      {isLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}
                    </Button>
                  </div>
                </form>
                <p className="mt-2 text-center text-xs text-muted-foreground">
                  Ontario legal information only. For legal advice, consult an Ontario lawyer.
                </p>
              </div>
            </div>
          </>
